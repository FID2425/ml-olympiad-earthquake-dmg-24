...
apply_algorithm <- function(algorithm, tuneGrid, train_data, test_data, model_path, predictions_path) {
  ...
  model <- train(
    ...
    method = algorithm,
    trControl = trainControl(method = "cv", number = 2, verboseIter = TRUE, classProbs = TRUE),
...
rf_tuneGrid <- expand.grid(.mtry = c(2, 3, 4, 5, 6, 7, 8, 9, 10),
                        .splitrule = "gini",
                        .min.node.size = c(1, 5, 10, 15, 20))

# Define the model and predictions paths
rf_model_path <- "../models/rf_onehot_model.RData"
rf_predictions_path <- "../data/predictions/rf_onehot_predictions.csv"

# Apply the algorithm
rf_model <- apply_algorithm("ranger", rf_tuneGrid, onehot_train, onehot_test, rf_model_path, rf_predictions_path)
```
...
rf_onehot_predictions <- read.csv(rf_predictions_path)

# Get the metrics
metrics <- get_prediction_metrics(onehot_test, rf_onehot_predictions)
metrics$confusion
...
```{r}
# Get the AUC
auc <- get_AUC_multiclass(onehot_test, rf_onehot_predictions)
```
...
# Define the model and predictions paths
gbm_model_path <- "../models/gbm_onehot_model.RData"
gbm_predictions_path <- "../data/predictions/gbm_onehot_predictions.csv"

# Apply the algorithm
gbm_model <- apply_algorithm("xgbTree", gbm_tuneGrid, onehot_train, onehot_test, gbm_model_path, gbm_predictions_path)
```
...
```{r}
# Load the predictions
gbm_onehot_predictions <- read.csv(gbm_predictions_path)

# Get the metrics
metrics <- get_prediction_metrics(onehot_test, gbm_onehot_predictions)
metrics$confusion
metrics$metrics_table
```
...
```{r}
# Get the AUC
auc <- get_AUC_multiclass(onehot_test, gbm_onehot_predictions)
```
...
# Define the model and predictions paths
sgb_model_path <- "../models/sgb_onehot_model.RData"
sgb_predictions_path <- "../data/predictions/sgb_onehot_predictions.csv"

# Apply the algorithm
sgb_model <- apply_algorithm("gbm", sgb_tuneGrid, onehot_train, onehot_test, sgb_model_path, sgb_predictions_path)
```
...
```{r}
# Load the predictions
sgb_onehot_predictions <- read.csv(sgb_predictions_path)

# Get the metrics
metrics <- get_prediction_metrics(onehot_test, sgb_onehot_predictions)
metrics$confusion
metrics$metrics_table
```
...
```{r}
# Get the AUC
auc <- get_AUC_multiclass(onehot_test, sgb_onehot_predictions)
```
