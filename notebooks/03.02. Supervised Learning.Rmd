...
# Get the metrics
metrics_rf_filtered <- get_prediction_metrics(filtered_onehot_test, rf_filtered_onehot_predictions)
metrics_rf_filtered$confusion
metrics_rf_filtered$metrics_table
```
...
```{r}
# Get the AUC
auc_rf_filtered <- get_AUC_multiclass(filtered_onehot_test, rf_filtered_onehot_predictions)
```
...
# Get the metrics
metrics_gbm_filtered <- get_prediction_metrics(filtered_onehot_test, gbm_filtered_predictions)
metrics_gbm_filtered$confusion
metrics_gbm_filtered$metrics_table
```
...
```{r}
# Get the AUC
auc <- get_AUC_multiclass(filtered_onehot_test, gbm_filtered_predictions)
```
...
# Get the metrics
metrics_sgb_filtered <- get_prediction_metrics(filtered_onehot_test, sgb_filtered_onehot_predictions)
metrics_sgb_filtered$confusion
metrics_sgb_filtered$metrics_table
```
...
```{r}
# Get the AUC
auc_sgb_filtered <- get_AUC_multiclass(filtered_onehot_test, sgb_filtered_onehot_predictions)
```
...
# Get the metrics
metrics_rf_weighted <- get_prediction_metrics(weighted_test, rf_weighted_predictions)
metrics_rf_weighted$confusion
metrics_rf_weighted$metrics_table
```
...
```{r}
# Get the AUC
auc_rf_weighted <- get_AUC_multiclass(weighted_test, rf_weighted_predictions)
```
...
x1 <- as.numeric(metrics_rf_weighted$metrics_table$F1[1])
x2 <- as.numeric(metrics_rf_weighted$metrics_table$F1[2])
x3 <- as.numeric(metrics_rf_weighted$metrics_table$F1[3])
