...
data <- read.csv("../data/raw/train.csv", sep = ",", header = TRUE)
...
test <- read.csv("../data/raw/test.csv", sep = ",", header = TRUE)
...
onehot_data <- read.csv(
  "../data/processed/onehot_train.csv",
  sep = ",",
  header = TRUE
)
...
onehot_test <- read.csv("../data/processed/onehot_test.csv",
  sep = ",",
  header = TRUE
)

pca_train <- read.csv("../data/processed/pca_train.csv",
  sep=",",
  header=TRUE
)
pca_train$damage_grade <- NULL
```
...
pca_three_dim$damage_grade <- train_damage_grade
...
fviz_nbclust(tsne_data$Y, kmeans, method = "silhouette", k.max = 15) +
  labs(subtitle = "Silhouette method")
# max silhouette values at 14 clusters
```

```{r}
# K-Means Clustering with 14 clusters
km_puntos_tsne <- kmeans(tsne_data$Y, centers = 14)
km_puntos_tsne
...
dbscan_tsne <- dbscan(tsne_data$Y, eps = 2.75, minPts = 6)
...
From the analysis conducted, several important conclusions can be drawn:
  - **Clustering Insights**: The clustering analysis based on the original data reveals two primary clusters, primarily driven by the age of the buildings. These variables appear to be strong indicators of the damage grade, suggesting that these features significantly contribute to the categorization of buildings based on their condition.
  - **Dimensionality Reduction Findings**: Visualization using dimensionality reduction techniques, such as PCA and t-SNE, highlights the complexity of the data. It is evident that the data is not linearly separable, which explains the suboptimal performance of simpler clustering algorithms.
    ...
    - **t-SNE Analysis**: t-SNE further complicates the scenario, revealing 14 distinct clusters, but with no clear correlation to damage grades.