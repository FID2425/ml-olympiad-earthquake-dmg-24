...
This project presents a comprehensive analysis of earthquake damage, focusing on the aftermath of the 2015 Gorkha earthquake in Nepal, which resulted in significant destruction and loss of life. The dataset utilized comprises detailed information on 4,000 buildings affected by the earthquake, encompassing various construction types such as reinforced concrete, brick masonry, and stone masonry. Key attributes include building age, height, land surface condition, house position, plan configuration, foundation type, ground floor type, roof type, and superstructure type. Through a systematic approach involving exploratory data analysis (EDA), data cleaning, and advanced machine learning techniques, we aimed to identify the factors influencing building damage and recovery needs. The methodology included dimensionality reduction techniques, such as Principal Component Analysis (PCA) and autoencoders, to visualize high-dimensional data and other techniques like SMOTE or weigthed classification. However, challenges such as low data interpretability, low feature variability, and unbalanced class distributions hindered predictive performance and limited insights, particularly for distinguishing between damage grades 2 and 3. Dimensionality reduction techniques and clustering methods, including t-SNE, revealed some building groupings but failed to correlate strongly with the target variable due to insufficient granularity in the data. These limitations highlighted the importance of integrating additional features, such as environmental and geographic data, to enhance model accuracy and interpretability. Despite these challenges, the study provides a foundation for improving earthquake damage prediction and recovery strategies using data-driven approaches.
...
<center>![](../docs/images/elbow k-means.png)</center>
...
<center>![](../docs/images/silhouette k-means.png)</center>

</div>

<div>

<center><b>Figure 3:</b> Silhouette method for K-Means clustering</center>
...
<center><b>Figure 4:</b> Parallel coordinates plot for K-Means clusters</center>
...
<center><b>Figure 5:</b> Dendogram for `complete` hierarchical clustering method</center>
...
<center><b>Figure 6:</b> Autoencoder performance during training</center>
...
In our study, we applied t-SNE to the onehot dataset to reduce its dimensions to 3, allowing for effective visualization of the data. The resulting plots demonstrated a complex distribution with several clusters; however, these clusters did not correlate with the damage_grade variable contrary to the other techniques applied. This finding suggests that t-SNE is to be applied on different contexts to other dimensionality reduction techniques and may be used in parallel to extract additional insights from the data.
